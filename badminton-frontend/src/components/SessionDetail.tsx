            ...
            <Box textAlign="center">
              <Typography variant="h5" color="warning.main">
                {session.total_shots > 0
                  ? ((session.in_box_shots / session.total_shots) * 100).toFixed(1)
                  : 0}
                %
              </Typography>
              <Typography variant="body2" color="text.secondary">
            ...
            <CardContent>
              ...
              <Box sx={{ mt: 3, p: 2, bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 1 }}>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1, mt: 1 }}>
                  ...
                  <Typography variant="body2">
                    <strong>In Box:</strong> {session.shots[selectedShotIndex].in_box ? '✅ Yes' : '❌ No'}
                  </Typography>
                </Box>
              </Box>
            </CardContent>
                  ...
                  <TableBody>
                    {session.shots.map((shot, index) => (
                      ...
                      >
                        ...
                        <TableCell align="center">
                          <Chip
                            label={shot.in_box ? 'YES' : 'NO'}
                            size="small"
                            color={shot.in_box ? 'success' : 'default'}
                          />
                        </TableCell>
                      </TableRow>
                    ))}
